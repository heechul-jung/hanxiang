...
function M.ThirtySixCrop(size)
   ...
   return function(input)
      ...
      if w > h then
	      short_size = h
              start_x[2] = w/2 - short_size / 2
              start_x[3] = w - short_size
      else
              short_size = w
              start_y[2] = h/2 - short_size / 2
              start_y[3] = h - short_size
      end
      w = short_size
      h = short_size

      for i=1,3 do      
	      crop_input = image.crop(input, start_x[i], start_y[i], start_x[i] + short_size, start_y[i] + short_size)
img = crop_input
--	      for _, img in ipairs{crop_input, image.hflip(crop_input)} do
        	 table.insert(output, centerCrop(img))
	         table.insert(output, image.crop(img, 0, 0, size, size))
        	 table.insert(output, image.crop(img, w-size, 0, w, size))
	         table.insert(output, image.crop(img, 0, h-size, size, h))
	         table.insert(output, image.crop(img, w-size, h-size, w, h))
	         table.insert(output, image.scale(img, size, size, 'bicubic'))
--	      end
      end
      -- View as mini-batch
      for i, img in ipairs(output) do
...
function M.ThirtySixCropFlip(size)
   local centerCrop = M.CenterCrop(size)
